...
def _remuneration_pivot(frame: pd.DataFrame) -> pd.DataFrame:
    col_ap = SALARY_AP_COL
    col_hp = SALARY_HP_COL

    data = frame[[YEAR_COL, GENDER_COL]].copy()
    data[col_ap] = (
        pd.to_numeric(frame.get(col_ap), errors="coerce") if col_ap in frame.columns else pd.Series([float("nan")] * len(frame))
    )
    data[col_hp] = (
        pd.to_numeric(frame.get(col_hp), errors="coerce") if col_hp in frame.columns else pd.Series([float("nan")] * len(frame))
    )

    grouped = data.groupby([YEAR_COL, GENDER_COL])[[col_ap, col_hp]].mean()
    per_year = data.groupby([YEAR_COL])[[col_ap, col_hp]].mean()

    years = sorted(grouped.index.get_level_values(0).unique())
    genders = sorted(grouped.index.get_level_values(1).unique())
    ...
    result = pd.DataFrame(matrix, index=pd.MultiIndex.from_tuples(columns, names=[YEAR_COL, GENDER_COL])).T
    result = result.sort_index(axis=1, level=[0, 1])
    return result


def build_remuneration_sheets(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    sheets: Dict[str, pd.DataFrame] = {}
    if (SALARY_AP_COL not in df.columns) and (SALARY_HP_COL not in df.columns):
        return sheets

    try:
        sheets[REMUNERATION_SHEET_NAME] = _remuneration_pivot(df)
    except Exception:
    ...
    if REGION_FOREIGN_COL in df.columns:
        ...
        try:
            sheets[REMUNERATION_FR_SHEET_NAME] = _remuneration_pivot(df_france)
        except Exception:
